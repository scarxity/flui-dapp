...
const MIN_TARGET_MIST = MIST_PER_SUI / BigInt(100);
...
export default function CreateEvent() {
  ...
  const compressFile = async (
    file: File,
    { maxSizeMB = 5, maxWidthOrHeight = 1600, fileType = "image/webp" } = {},
  ): Promise<File> => {
    const options = {
      maxSizeMB,
      maxWidthOrHeight,
    ...
    const ext = fileType === "image/webp" ? "webp" : file.name.split(".").pop();
    return new File(
      [compressedBlob],
  ...
  const uploadImageToIPFS = async () => {
    try {
      ...
      const compressed = await compressFile(image, {
        maxSizeMB: 0.6,
      ...
      const fileExtension = compressed.name.split(".").pop();
      const uniqueFilename = `event-${timestamp}-${randomString}.${fileExtension}`;

      const renamedFile = new File([compressed], uniqueFilename, {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (targetAmount < MIN_TARGET_MIST) {
      showErrorToast("Target fund must be at least 0.01 SUI.");
      return;
    }
